# -*- coding: utf-8 -*-
"""
将语文知识卡导入 Anki（通过 AnkiConnect）
答案中包含 webp 配图
优化：
1. 图片复制到 Anki 媒体文件夹，Back 中通过 <img> 引用；
2. 使用 addNotes 批量导入；
3. 导入前先清空目标牌组中的现有卡片。
"""
import os
...
def parse_cards(md_path):
    ...
    for i in range(1, len(parts), 2):
        ...
        e_match = re.search(r"\*\*易错分析：\*\*(.*?)(?=\*\*记忆钩子：\*\*)", block, re.S)
...
def build_note(card, md_path, media_dir):
    ...
    back_parts = []
    if card["answer"]:
        back_parts.append(f"<div><b>答案：</b><br>{card['answer'].replace(chr(10), '<br>')}</div>")
    if card["error_analysis"]:
        back_parts.append(f"<div style='margin-top:10px;color:#c00;'><b>易错分析：</b><br>{card['error_analysis'].replace(chr(10), '<br>')}</div>")
    if card["hook"]:
        back_parts.append(f"<div style='margin-top:10px;color:#080;'><b>记忆钩子：</b>{card['hook']}</div>")
    if img_html:
...
def main():
    ...
    md_files = [p for p in md_files if "配图缺失清单" not in p.name and "提示词" not in p.name]
